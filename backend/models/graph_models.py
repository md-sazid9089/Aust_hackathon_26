...
class GraphEdge(BaseModel):
    ...
    base_travel_time_s: float = Field(0.0, description="Base traversal time without anomalies")
    road_type: str = Field("unknown", description="OSM highway tag value")
    speed_limit_kmh: Optional[float] = None
    anomaly_multiplier: float = Field(1.0, description="Current anomaly weight multiplier (1.0 = normal)")
    ml_predicted: bool = Field(False, description="Whether travel_time uses ML prediction")
